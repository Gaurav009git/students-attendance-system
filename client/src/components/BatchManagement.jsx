...
function BatchManagement() {
    ...
    const fetchData = async () => {
        try {
            ...
            // Fetch all data simultaneously using adminService
            const [branchesData, teachersData, batchesData] = await Promise.all([
    ...
    const handleEdit = (batch) => {
        ...
        setFormData({
            ...
            year: batch.year,
            semester: batch.semester,
            classTeacher: batch.classTeacher?._id || batch.classTeacher || ''
        });
        setShowForm(true);
    };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (editingBatch) {
                await adminService.updateBatch(editingBatch._id, submitData);
                setMessage({ type: 'success', text: 'Batch updated successfully!' });
            } else {
                await adminService.createBatch(submitData);
                setMessage({ type: 'success', text: 'Batch created successfully!' });
            }
            ...
            console.error('Submit error:', error);
            const errorMessage = error.response?.data?.error || error.response?.data?.message || 'Failed to save batch';
            setMessage({ 
                type: 'danger', 
                text: errorMessage
            });
    ...
    const handleDelete = async (batchId) => {
        ...
        try {
            ...
            const errorMessage = error.response?.data?.error || 'Failed to deactivate batch';
            setMessage({ 
                type: 'danger', 
                text: errorMessage
            });
